...
## Lista de Shares
ForEach ($Sharename in (Get-Content -path "C:\temp\shares.txt")){
...
$Shares = Get-WmiObject -ComputerName $Server -Class win32_share -Filter "name = '$sharename'" | Select-Object Name -ExpandProperty Name
...
$err.Exception.Message | Out-File $ExportPath\ErrorColeta.txt -Append
}
